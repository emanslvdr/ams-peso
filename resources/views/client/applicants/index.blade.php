    ...
    <div class="max-w-7xl mx-auto p-6">
        <div class="bg-white shadow rounded-xl">
            <div class="flex justify-between items-center px-6 py-4 bg-gray-50 rounded-t-xl">
                <h2 class="text-lg font-semibold">All Applicants</h2>
            </div>
...
<tbody>
    ...
    <tr class="border-t hover:bg-blue-50 transition">
        <td class="px-4 py-2">
            ...
            @else
                <span class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-base text-blue-600 font-bold">
                    {{ strtoupper(substr(optional($application->user)->name ?? 'A', 0, 1)) }}
                </span>
            @endif
        </td>
        <td class="px-4 py-2 font-semibold">
            {{ $application->first_name ?? optional($application->user)->name }}
        ...
        @php
            $applicantData = [
                'name'         => trim(($application->first_name ?? '') . ' ' . ($application->last_name ?? '')) ?: optional($application->user)->name,
                ...
                'education'    => $application->institution ?? '',
                'profile_photo'=> ($application->user && $application->user->profile_photo) ? asset('storage/'.$application->user->profile_photo) : null,
            ];
        @endphp
        <td class="px-4 py-2">
            <button
                class="flex items-center text-blue-600 hover:text-blue-800 text-xs gap-1"
                onclick='showApplicantModal(@json($applicantData))'
                title="View Details"
            >
...
<div class="bg-white shadow rounded-xl mt-8">
    ...
    @else
        <div class="p-8 flex flex-col items-center text-gray-400">
            ...
            <span>All users have submitted applications.</span>
        </div>
    @endif
</div>
        ...
        <div id="applicantModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 opacity-0 invisible transition-opacity duration-300">
            <div class="bg-white rounded-xl shadow-2xl max-w-md w-full px-8 py-7 relative">
                ...
                <div class="flex flex-col items-center mb-3">
                    <span id="modalProfilePhoto"></span>
                    <div class="text-xl font-semibold mt-2 mb-1" id="modalName"></div>
                    <div class="text-sm text-gray-500 mb-1" id="modalEmail"></div>
                </div>
...
function showApplicantModal(data) {
    ...
    document.getElementById('modalEducation').innerText = data.education || '-';
    document.getElementById('modalAppliedAt').innerText = data.applied_at || '-';
    let photo = document.getElementById('modalProfilePhoto');
    if(data.profile_photo){
        photo.innerHTML = `<img src="${data.profile_photo}" class="w-16 h-16 rounded-full border object-cover shadow mb-2">`;
    } else {
        photo.innerHTML = `<span class="w-16 h-16 rounded-full bg-blue-100 flex items-center justify-center text-2xl text-blue-600 font-bold mb-2">${(data.name||'A').charAt(0)}</span>`;
    }

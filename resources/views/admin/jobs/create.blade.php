<x-admin-layout>
    <div class="max-w-lg mx-auto my-8">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ url()->previous() }}"
                class="inline-flex items-center px-4 py-2 border border-gray-300 rounded hover:bg-blue-50 text-gray-700 text-sm">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7"/></svg>
                Back
            </a>
        </div>

        <form action="{{ route('jobs.store') }}" method="POST" class="bg-white p-8 shadow-lg rounded-xl border space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="organization_id" required
                    class="w-full border-gray-300 rounded p-2 focus:ring focus:ring-blue-100 focus:border-blue-400"
                    @if(request('organization_id') && $organizations->count() === 1) disabled @endif>
                ...
                @if(request('organization_id') && $organizations->count() === 1)
            ...
            <div>
                <button type="submit" class="w-full py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition">Post Job</button>
            </div>
        </form>
    </div>
</x-admin-layout>

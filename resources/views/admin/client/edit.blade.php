...
<div class="max-w-xl mx-auto p-6">
    ...
    <form action="{{ route('clients.update', $client) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" class="w-full border rounded p-2" value="{{ old('name', $client->name) }}" required>
            @error('name') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block mb-1 font-medium">Email</label>
            <input type="email" name="email" class="w-full border rounded p-2" value="{{ old('email', $client->email) }}" required>
